...
def test_study_rule__defaults_unique_ids(create_study_rule_db, db_session):
    ...
    first = (
        ...
        .filter(studyrule.StudyRule.name == "first")
        .one()
    )
    second = (
        db_session.query(studyrule.StudyRule)
...
def test_study_rule__defaults_created_datetime(create_study_rule_db, db_session):
    ...
    s = (
        db_session.query(studyrule.StudyRule)
        .filter(studyrule.StudyRule.name == "asdf")
        .one()
    )
...
def test_study_rule__updates_updated_datetime(create_study_rule_db, db_session):
    ...
    updated = (
        db_session.query(studyrule.StudyRule)
        .filter(studyrule.StudyRule.id == 1618)
        .one()
    )
...
def test_study_rule__whole_programs(create_study_rule_db, db_session):
    ...
    assert all(
        [program.study_rule_id == 1618 for program in first_study.whole_programs]
    )

    second_study = (
        db_session.query(studyrule.StudyRule)
        .filter(studyrule.StudyRule.id == 3141)
        .one()
    )
...
def test_study_rule__partial_programs(create_study_rule_db, db_session):
    ...
    assert all(
        [program.study_rule_id == 1618 for program in first_study.partial_programs]
    )
...
def test_study_rule__to_json_without_dates():
    s = studyrule.StudyRule(id=1618, name="asdf",)
    expected = json.loads(
        json.dumps(
            {
                "id": 1618,
...
def test_study_rule_program__study_rule_must_exist(create_study_rule_db, db_session):
    with pytest.raises(
        exc.IntegrityError, match=r"study_rule_program_study_rule_id_fk"
    ):
        db_session.add(
...
def test_study_rule_program__study_rule_id_must_be_defined(
...
def test_study_rule_program__program_name_must_be_defined(
    create_study_rule_db, db_session
):
    with pytest.raises(exc.IntegrityError, match=r"violates not-null constraint"):
...
def test_study_rule_program__uniqueness(create_study_rule_db, db_session):
    ...
    with pytest.raises(exc.IntegrityError, match=r"study_rule_program_pk"):
        db_session.add(
            studyrule.StudyRuleProgram(study_rule_id=1618, program_name="dupe")
        )
        db_session.commit()


def test_study_rule_program__defaults_created_datetime(
    create_study_rule_db, db_session
):
    db_session.add(studyrule.StudyRule(id=1618, name="asdf"))
    db_session.commit()
...
def test_study_rule_program__defaults_updated_datetime(
    create_study_rule_db, db_session
):
    db_session.add(studyrule.StudyRule(id=1618, name="asdf"))
    db_session.commit()
...
def test_study_rule_program__to_json_without_dates():
    sp = studyrule.StudyRuleProgram(study_rule_id=1618, program_name="alpha",)
    expected = json.loads(
        json.dumps(
            {
                "study_rule_id": 1618,
...
def test_study_rule_program_project__study_rule_id_must_be_defined(
    ...
    with pytest.raises(exc.IntegrityError, match=r"violates not-null constraint"):
        db_session.add(
            studyrule.StudyRuleProgramProject(
                program_name="game_of_life", project_code="asdf"
            )
        )
        db_session.commit()
...
def test_study_rule_program_project__project_code_must_be_defined(
    ...
    with pytest.raises(exc.IntegrityError, match=r"violates not-null constraint"):
        db_session.add(
            studyrule.StudyRuleProgramProject(
                study_rule_id=1618, program_name="game_of_life"
            )
        )
        db_session.commit()


def test_study_rule_program_project__study_rule_must_exist(
    create_study_rule_db, db_session
):
    with pytest.raises(
