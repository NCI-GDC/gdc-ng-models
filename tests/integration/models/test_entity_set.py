...
def test_entity_set_create(create_entity_set_db, db_session):
    ...
    object_under_test = create_nominal_entity_set()
    db_session.add(object_under_test)
    db_session.commit()

    assert object_under_test.id == STRING_128_CHAR
    assert object_under_test.type == entity_set.SetType.frozen
    assert object_under_test.entity_type == entity_set.EntityType.case
    assert object_under_test.entity_ids == [STRING_36_CHAR]
    assert object_under_test.accessed_datetime
    assert object_under_test.created_datetime
    assert object_under_test.updated_datetime
...
def test_entity_set_id_too_long(create_entity_set_db, db_session):
    ...
    object_under_test = create_nominal_entity_set()
    object_under_test.id = STRING_129_CHAR
    with pytest.raises(Exception):
        db_session.add(object_under_test)
        db_session.commit()
...
def test_entity_set_entity_type_cannot_be_none(create_entity_set_db, db_session):
    ...
    object_under_test = create_nominal_entity_set()
    object_under_test.entity_type = None
    with pytest.raises(Exception):
        db_session.add(object_under_test)
        db_session.commit()
...
def test_entity_set_entity_ids_cannot_be_none(create_entity_set_db, db_session):
    ...
    object_under_test = create_nominal_entity_set()
    object_under_test.entity_ids = None
    with pytest.raises(Exception):
        db_session.add(object_under_test)
        db_session.commit()
        ...
        json.dumps(
            {
                ...
                "created_datetime": object_under_test.created_datetime.isoformat(),
                "updated_datetime": object_under_test.updated_datetime.isoformat(),
                "accessed_datetime": object_under_test.accessed_datetime.isoformat(),
                "time_to_live_sec": None,
            }
        )
    )
    assert object_under_test.to_json() == expected_json
