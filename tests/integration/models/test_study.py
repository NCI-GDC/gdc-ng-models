...
def test_study_rule__defaults_unique_ids(create_study_rule_db, db_session):
    ...
    first = (
        db_session.query(studyrule.StudyRule).filter(studyrule.StudyRule.name == "first").one()
    )
    second = (
        db_session.query(studyrule.StudyRule)
        .filter(studyrule.StudyRule.name == "second")
        .one()
    )
...
def test_study_rule__updates_updated_datetime(create_study_rule_db, db_session):
    ...
    db_session.query(studyrule.StudyRule).filter(studyrule.StudyRule.id == 1618).update(
        {"name": "updated"}
    )
    db_session.commit()

    updated = (
        db_session.query(studyrule.StudyRule).filter(studyrule.StudyRule.id == 1618).one()
    )
...
def test_study_rule__whole_programs(create_study_rule_db, db_session):
    ...
    second_study = (
        db_session.query(studyrule.StudyRule).filter(studyrule.StudyRule.id == 3141).one()
    )
...
def test_study_rule__partial_programs(create_study_rule_db, db_session):
    ...
    db_session.add(
        studyrule.StudyRuleProgramProject(
            study_rule_id=1618, program_name="alpha", project_code="uno"
        )
    )
    db_session.add(
        studyrule.StudyRuleProgramProject(
            study_rule_id=1618, program_name="alpha", project_code="dos"
        )
    )
    db_session.commit()
...
def test_study_rule__str_without_date():
    ...
    expected = (
        "<StudyRule(id=1618, name='asdf', created_datetime=None, updated_datetime=None)>"
    )
    assert str(s) == expected
...
def test_study_rule_program__study_rule_must_exist(create_study_rule_db, db_session):
    with pytest.raises(exc.IntegrityError, match=r"study_rule_program_study_rule_id_fk"):
        db_session.add(
            studyrule.StudyRuleProgram(study_rule_id=1618, program_name="game_of_life")
        )
        db_session.commit()


@pytest.mark.filterwarnings(
    "ignore:Column 'study_rule_program.study_rule_id' is marked as a member of the primary key"
    " for table 'study_rule_program'"
)
...
@pytest.mark.filterwarnings(
    "ignore:Column 'study_rule_program.program_name' is marked as a member of the primary key "
    "for table 'study_rule_program'"
)
...
def test_study_rule_program__study_rule_can_contain_multiple_programs(
    create_study_rule_db, db_session
):
    db_session.add(studyrule.StudyRule(id=1618, name="asdf"))
    db_session.commit()
    ...
    assert (
        db_session.query(studyrule.StudyRuleProgram)
        .filter(studyrule.StudyRuleProgram.study_rule_id == 1618)
        .count()
        == 2
    )
...
def test_study_rule_program__defaults_created_datetime(create_study_rule_db, db_session):
    ...
    s = (
        db_session.query(studyrule.StudyRuleProgram)
        .filter(
            studyrule.StudyRuleProgram.study_rule_id == 1618
            and studyrule.StudyRuleProgram.program_name == "alpha"
        )
        .one()
    )
    assert s.created_datetime is not None
...
def test_study_rule_program__updates_updated_datetime_different(
    create_study_rule_db, db_session
):
    db_session.add(studyrule.StudyRule(id=1618, name="asdf"))
    db_session.commit()
    ...
    db_session.query(studyrule.StudyRuleProgram).filter(
        studyrule.StudyRuleProgram.study_rule_id == 1618
...
def test_study_rule_program__str():
    ...
    expected = (
        "<StudyRuleProgram(study_rule_id=1618, program_name='alpha', "
        "created_datetime=2020-04-15T12:45:21.000123+00:00, "
        "updated_datetime=2020-04-16T12:45:21.000123+00:00)>"
    )
...
@pytest.mark.filterwarnings(
    "ignore:Column 'study_rule_program_project.study_rule_id' is marked as a member of the "
    "primary key for table 'study_rule_program_project'"
)
def test_study_rule_program_project__study_rule_id_must_be_defined(
    create_study_rule_db, db_session
):
    with pytest.raises(exc.IntegrityError, match=r"violates not-null constraint"):
        db_session.add(
            studyrule.StudyRuleProgramProject(program_name="game_of_life", project_code="asdf")
        )
        db_session.commit()


@pytest.mark.filterwarnings(
    "ignore:Column 'study_rule_program_project.program_name' is marked as a member of the "
    "primary key for table 'study_rule_program_project'"
)
def test_study_rule_program_project__program_name_must_be_defined(
    create_study_rule_db, db_session
):
    with pytest.raises(exc.IntegrityError, match=r"violates not-null constraint"):
        db_session.add(
            studyrule.StudyRuleProgramProject(study_rule_id=1618, project_code="asdf")
        )
        db_session.commit()


@pytest.mark.filterwarnings(
    "ignore:Column 'study_rule_program_project.project_code' is marked as a member of the "
    "primary key for table 'study_rule_program_project'"
)
def test_study_rule_program_project__project_code_must_be_defined(
    create_study_rule_db, db_session
):
    with pytest.raises(exc.IntegrityError, match=r"violates not-null constraint"):
        db_session.add(
            studyrule.StudyRuleProgramProject(study_rule_id=1618, program_name="game_of_life")
        )
        db_session.commit()
...
def test_study_rule_program_project__study_rule_must_exist(create_study_rule_db, db_session):
    with pytest.raises(
        exc.IntegrityError, match=r"study_rule_program_project_study_rule_id_fk"
    ):
        db_session.add(
            studyrule.StudyRuleProgramProject(
                study_rule_id=1618, program_name="game_of_life", project_code="asdf"
            )
        )
        db_session.commit()
...
def test_study_rule_program_project__uniqueness(create_study_rule_db, db_session):
    """Check table enforces uniqueness of the study_rule_id, program_name, & project_code."""
    db_session.add(studyrule.StudyRule(id=1618, name="asdf"))
    db_session.commit()

    db_session.add(
        studyrule.StudyRuleProgramProject(
            study_rule_id=1618, program_name="dupe", project_code="dupe"
        )
    )
    db_session.commit()
...
def test_study_rule_program_project__study_rule_can_have_multiple_programs_and_projects(
    create_study_rule_db, db_session
):
    db_session.add(studyrule.StudyRule(id=1618, name="asdf"))
    db_session.commit()
    ...
    db_session.add(
        studyrule.StudyRuleProgramProject(
            study_rule_id=1618, program_name="beta", project_code="uno"
        )
    )
    db_session.add(
        studyrule.StudyRuleProgramProject(
            study_rule_id=1618, program_name="beta", project_code="dos"
        )
    )
    db_session.commit()

    assert (
        db_session.query(studyrule.StudyRuleProgramProject)
...
def test_study_rule_program_project__defaults_created_datetime(
    create_study_rule_db, db_session
):
    db_session.add(studyrule.StudyRule(id=1618, name="asdf"))
    db_session.commit()
    ...
    s = (
        db_session.query(studyrule.StudyRuleProgramProject)
        .filter(
            studyrule.StudyRuleProgramProject.study_rule_id == 1618
...
def test_study_rule_program_project__defaults_updated_datetime(
    create_study_rule_db, db_session
):
    db_session.add(studyrule.StudyRule(id=1618, name="asdf"))
    db_session.commit()
...
def test_study_rule_program_project__str():
    ...
    expected = (
        "<StudyRuleProgramProject(study_rule_id=1618, program_name='alpha', project_code="
        "'uno', created_datetime=2020-04-15T12:45:21.000123+00:00, updated_datetime=2020-04-16"
        "T12:45:21.000123+00:00)>"
    )
...
def test_study_rule_program_project__str_without_dates():
    ...
    expected = (
        "<StudyRuleProgramProject(study_rule_id=1618, program_name='alpha', project_code='uno'"
        ", created_datetime=None, updated_datetime=None)>"
    )
    assert str(spp) == expected
