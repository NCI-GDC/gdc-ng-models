...
@pytest.mark.filterwarnings(
    "ignore:Column 'study_rule_program.study_rule_id' is marked as a member of the primary "
    "key for table 'study_rule_program'"
)
...
@pytest.mark.filterwarnings(
    "ignore:Column 'study_rule_program.program_name' is marked as a member of the primary "
    "key for table 'study_rule_program'"
)
...
def test_study_rule_program_project__uniqueness(create_study_rule_db, db_session):
    """Check that table enforces uniqueness of study_rule_id, program_name, & project_code."""
    db_session.add(studyrule.StudyRule(id=1618, name="asdf"))
    db_session.commit()
...
def test_study_rule_program_project__updates_updated_datetime(
    create_study_rule_db, db_session
):
    db_session.add(studyrule.StudyRule(id=1618, name="asdf"))
    db_session.commit()
...
def test_study_rule_program_project__str():
    ...
    expected = (
        "<StudyRuleProgramProject(study_rule_id=1618, program_name='alpha', "
        "project_code='uno', created_datetime=2020-04-15T12:45:21.000123+00:00, "
        "updated_datetime=2020-04-16T12:45:21.000123+00:00)>"
    )
...
def test_study_rule_program_project__str_without_dates():
    ...
    expected = (
        "<StudyRuleProgramProject(study_rule_id=1618, program_name='alpha', "
        "project_code='uno', created_datetime=None, updated_datetime=None)>"
    )
    assert str(spp) == expected
