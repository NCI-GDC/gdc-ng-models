...
class RedactionLog(Base):
    ...
    def is_rescinded(self) -> bool:
        ...
        Returns:
           True if all are rescinded; otherwise, false.
        """
        for entry in self.entries:
...
class RedactionEntry(Base):
    """Logs a redacted node, holds enough information to enable querying & filtering."""
    ...
    redaction_id = Column(
        BigInteger, ForeignKey("redaction_log.id"), nullable=False, primary_key=True
    )
